...
// Atom to track if processing is cancelled
...
export const processUrlAtom = atom(
	...
	(
		...
		const isCancelled = get(isCancelledAtom)
		if (isCancelled) {
			return null
		}
...
export const deleteUrlsAtom = atom(null, (get, set, urlIds: string[]) => {
	const currentUrls = get(urlsAtom)
	const filteredUrls = currentUrls.filter((url) => !urlIds.includes(url.id))
	set(urlsAtom, filteredUrls)
})

export const cleanupQueuedUrlsAtom = atom(null, (get, set, urls: string[]) => {
	const currentUrls = get(urlsAtom)
	const filteredUrls = currentUrls.filter(
		(url) => !urls.includes(url.url) || url.status !== 'queued',
	)
	set(urlsAtom, filteredUrls)
})
...
export const rerunUrlsWithQueueAtom = atom(
	...
	(
		...
		set,
		{
			urlIds,
			addToQueue,
		}: { urlIds: string[]; addToQueue: (urls: string[]) => void },
	) => {
		const currentUrls = get(urlsAtom)
